...
include::../../../../percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentEncoderImpl.java[lines=78..80]
}
----
...
----
<1> SPI(Service Provider Interface) 를 통해 객체를 적재하고 제공한다.

===== encodeOctet(int, ByteBuffer)

다음과 같이 `encodeOctet(int, ByteBuffer)` 를 구현한다.

[source,java]
.PercentEncoderImpl.java#encodeOctet(int, ByteBuffer)
----
include::../../../../percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentEncoderImpl.java[lines=54..76]
----
<1> 인코딩할 필요가 없는 술자는 그대로 버퍼에 담는다.
