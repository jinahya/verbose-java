...
<2> 입력 배열 크기의 절반 만큼의 길이를 갖는 출력 배열을 생성한다
<3> `decode(ByteBuffer, ByteBuffer)` 메서드에 전달해서 디코드한다.
<4> 출력 배열과 명시된 `charset` 을 이용하여 문자열을 생성한 후 반환한다.

==== HexOutputStream.java

지록된는 바이트들을 자동으로 `base16` 문자들로 바꾸어 기록하는 `OutputStream` 이다. {apidocs-javase}/java/io/FilterOutputStream.html[`java.io.FilterInputStream`] 클래스를 확장하여 작성되었다.
...
----
주의 할 점은 `flush()`, `close()` 등의 다른 메서드들도 `out` 필드가 `null` 인지 확인하지 않는다는 것이다.
[source,java]
...
`HexDecoder` 인터페이스를 이용하여 hex 문자들을 바이트로 변환해서 읽어들이는 `InputStream` 이다. {apidocs-javase}/java/io/FilterInputStream.html[`FilterInputStream`[ 을 확장하여 만들어졌다.
...
- {apidocs-javase}/java/io/FilterInputStream.html#read-byte:A-int-int-[`FilterInputStream#read(byte[\],int,int)`] 메서드는 `in` 필드에 직접 `read(byte[],int,int)` 메서드를 호출한다. 다음과 같이 재정의된 `read()` 메서드를 호출하도록 재정의되었다.
+
[source,java]
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=40..51]
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=62..65]
----
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=75..80]
----
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=88..95]
----
...
{apidocs-javase}/java/nio/channels/WritableByteChannel.html[`WritableByteChannel`] 인터페이스에 정의된 {apidocs-javase}/java/nio/channels/WritableByteChannel.html#write-java.nio.ByteBuffer-[`write(ByteBuffer)`] 메서드를 다음과 같이 구현하였다.
[source,java]
...
{apidocs-javase}/java/nio/channels/ReadableByteChannel.html[`ReadableByteChannel`] 인터페이스에 정의된 {apidocs-javase}/java/nio/channels/ReadableByteChannel.html#read-java.nio.ByteBuffer-[`read(ByteBuffer)`] 메서드를 다음과 같이 구현하였다.
[source,java]
...
----
include::../../../../hex-codec-lib/src/main/java/com/github/jinahya/verbose/hex/HexEncoderImpl.java[lines=36..49]
----
...
[CAUTION]
====
역시나 소문자로 가정한 부분이 있다. 다음과 같이 모든 경우에 대해 확실한 코드를 사용하는 편이 더 훌륭한 코드가 아닐까 한다.
[source,java]
----
            case 'a': //  97, 0x61
            case 'b': //  98, 0x62
...
----
include::../../../../hex-codec-lib/src/main/java/com/github/jinahya/verbose/hex/HexDecoderImpl.java[lines=44..55]
----
첫 번째 문자를 디코딩하고 윈쪽으로 `4` 만큼 옮긴 값에 두번째 문자를 디코딩한 값을 OR(`|`) 연산하여 반환한다.

[TIP]
====
만약 `decodeNibble(int)` 메서드가 네 개의 비트로 이루어진 숫자를 반환한다는 것이 보장된다면 다음과 같이 짧게 구현할 수도 있다.
[source,java]
----
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=92..106]
----
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=108..121]
----
<1> 버퍼에 읽혀진 모든 데이터를 기록한다.
<2> 버퍼에 남은 데이터가 없으므로 `buffer.clear()` 를 호출한 것과 같은 결과이다.
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=142..151]
----
...
[source,java]
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=153..167]
----

다음은 `trasferFrom` 메서드를 사용해서 파일을 복사하는 메서드이다.
[source,java]
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=169..183]
----
...
다음은 `InputStream` 객체와 알코리즘 이름을 받아서 값을 구하는 메서드이다.
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=51..57]
----
<1> 주어진 `input` 객제를 `DigestInputStream` 클래스로 감싼다.
<2> `input` 의 모든 데이터를 읽는다. (내부적으로 읽힌 데이터가 `digest` 에 갱신된다.)

아래는 파일을 메시지 다이제스트를 구하는 메서드이다.

[source,java]
.MdUtils.java#digest(File,String)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=69..74]
----
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=76..87]
----
<1> `channel` 에서 데이터를 읽어 버퍼에 채운다.
<2> `buffer` 에 채원진 데이터를 `digest` 에 갱신한다.

아래는 `Path` 객체의 메시지 다이제스트를 구하는 메서드이다.
...
// 최대 count 만큼만 복사한다. (count 가 -1L 이면 스트림의 끝까지 복사한다.)
...
// 최대 count 만큼만 복사한다. (count 가 -1L 이면 채널의 끝까지 복사한다.)
