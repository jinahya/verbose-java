...
그리고 (네 개의 비트로 구성된) 각 그룹의 값(`0` ~ `15`)을 아래 표에 정의된 열 여섯개(`2^4^`)의 문자 중 하나로 변경한다.
...
`HexDecoder` 인터페이스를 이용하여 `read(ByteBuffer)` 메서드에 전달된 버퍼에 디코딩된 바이트들을 저장하는 채널이다. 생성자와 `isOpen()`, `close()` 등의 메서드는 `WritableHexChannel` 과 동일한 방식으로 구현되었다. 아래는 `read(ByteBufffer)` 메서드의 내용이다.
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=93..132]
----
