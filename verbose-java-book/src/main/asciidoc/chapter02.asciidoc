...
본 장에서는 '숫자' 밖에 모르는 컴퓨터와 '문자' 를 사용하는 사람과의 사이에 종종 발생하는 문제점들을 해결하는 방법들을 살펴본다. 우선 자바의 자료형에 대해 간략하고 알아본 후 [http://tools.ietf.org/html/rfc4648#section-8[Base 16 Encoding (RFC 4648)]]과 [https://tools.ietf.org/html/rfc3986#section-2.1[Percent-Encoding (RFC 3986)]]에 대해 알아보자.
...
거의 모든 컴퓨터들은 자료를 저장할 때, `octet` 으로 불리는, 여덟개의 비트로 구성된 최소 단위로 나누어 저장한다. 'a' 라는 문자는 (거의 모든 경우에) `01100001` 로, '홍' 이라는 문자는 (`UTF-8` 로 인코딩 되었을 때) `111011011 00110011 0001101` 로 저장된다. 아래 표에 여덟개의 비트로 표현할 수 있는 몇몇 값들을 10진법, 8진법, 2진법, 그리고 자바의 `byte` 값으로 표현하였다.
...
그리고 (네 개의 비트로 구성된) 각 그룹의 값(`0` ~ `2^4^-1`)을 아래 표에 정의된 열 여섯개의 문자 중 하나로 변경한다.
...
hex-codec-api 모듈에는 `base16` 코덱을 위한 두 개의 인터페이스가 정의되어 있다.
...
`HexEncoder` 인터페이스에는 몇몇 `default` 메서드들이 정의되어 있다.
...
----
<1> `decoded` 버퍼에 읽을 바이트들이 님아 있고 `encoded` 버퍼에 최소한 두 개의 바이트를 기록할 수 있는 동안 `encode(int, ByteBuffer)` 를 호출한다.
...
----
<1> `encoded` 버퍼에 읽을 바이트의 개수가 2개 이상 있고 `decoded` 버퍼에 쓸 수 있는 공간이 남아 있는 동안 `decodeOctet(ByteBuffer)` 메서드에 `encoded` 버퍼를 전달하고 반환된 값을 `decoded` 버퍼에 담는다.
...
----
<1> `encoded` 버퍼에서 읽을 수 있는 바이트 개수의 절반 만큼의 버퍼를 생성하고 `encode(ByteBuffer, ByteBuffer)` 메서드에 전달해서 디코드한다.
<2> 반환할 버퍼를 뒤집어(flip) 곧바로 읽을 수 있는 상태로 만든다.
...
----
이후의 소스를 보면 다음과 같이 개선되었지만 여전히 `flush()` 메서드에서는 `out` 필드의 값을 확인하지 않고 사용한다.
[source,java]
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=118..122]
----
...
[CAUTION]
====
`available` 메서드가 반환하는 값을 최적의(optimal) 버퍼 크기로 오해하면 안된다. 몇몇 `InputStream` 들은 `available` 메서드에서 읽을 수 있는 모든 바이트들의 크기를 반환하다.
...
----
<1> `src` 파일의 크기가 가용한 메모리 용량보다 크면 문제가 발생한다.
====
+
[TIP]
====
굳이 `available()` 의 값을 사용하고 싶다면 다음과 같은 방법이 가능하다.
[source,java]
----
static final LEN = 4096; // default buffer length.

final int availalbe = in.available(); // <1>
final byte[] buf = new buf[available == 0 ? LEN : Math.min(LEN, available)];
----
<1> 이미 스트림의 끝(`end-of-stream`)에 도달한 상태라면 `0` 을 반환한다.
====

- `skip(long)` 메서드는 주어진 값보다 최대 1이 작은 짝수만큼 시도하고 홀수만큼 스킵되면 무조건 하나를 더 소비한다.
+
[source,java]
...
<2> 실제로 건너 띈 개수가 홀수이면 하나를 더 읽어서 버린다.
<3> 실제로 건너 띈 개수의 절반에 해당하는 값을 반환한다.

==== FilterChannel.java
이제 `HexOutputStream` 클래스와 `HexInputStream` 클래스와 동일한 기능을 수행하는 클래스들을  `java.nio.channels` 패키지에 정의된 클래스들을 사용하여 구현해 보자. `FilterOutputStream`, `FilterInputStream` 등의 기능을 위한 클래스들이 존재하지 않으므로 `WritableByteChannel` 과 `ReadableByteChanel` 을 직접 구현해야 한다. 다만 두 인터페이스가 공통으로 확장하는 `java.nio.channels.Channel` 인터페이스를 기반으로 추상클래스를 하나 정의하고 공통의 기능을 미리 구현하도록 하겠다.
...
public abstract class FilterChannel<T extends Channel, U> implements Channel {
    ...
    // channel : 실제 데이터가 읽히거나 기록될 채널
    // filter  : 인코딩/디코딩에 사용할 필터
    // capacity: 인코딩/디코딩에 사용할 버퍼의 용량
    // direct  : true 이면 버퍼를 생성할 때 다이렉트 버퍼를 생성한다
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=40..47]
...
`Channel` 인터페이스의 `isOpen()` 메서드는 `channel` 인스턴스의 `isOpen()` 을 호출한다. `channel` 객체가 `null` 일 경우에 대한 별도의 코드를 추가하지 않았다.
[source,java]
.FilterChannel.java#isOpen()
----
    // channel 이 null 이면 NullPointerException 이 발생한다.
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=58..61]
----

`Channel` 인터페이스의 `close()` 메서드는 `channel` 이 `null` 이 아닐 경우에 한해 `channel` 인스턴스에 직접 호출한다.
[source,java]
.FilterChannel.java#close()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=71..76]
----

`capacity` 와 `direct` 값을 사용하여 생성된 버퍼를 반환하는 메서드이다.
[source,java]
.FilterChannel.java#buffer()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=84..91]
----
...
----
<1> 인코딩에 사용할 버퍼의 크기는 최소한 2보다 커야 한다.

`WritableByteChannel` 인터페이스에 정의된 `write(ByteBuffer)` 메서드를 다음과 같이 구현하였다.
[source,java]
.WritableHexChannel.java#close()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=59..77]
----
<1> `filter(HexEncoder)` 를 이용해 `src` 의 내용을 `buffer()` 에 인코딩한다.
<2> 인코딩된 값을 `channel` 에 기록한다.
<3> `buffer` 에 남은 데이터가 한번에 모두 기록되지 않았다면 non-blocking mode 의 채널이라 가정하고 반복 구문을 빠져 나간다.
...
`ReadableByteChannel` 인터페이스에 정의된 `read(ByteBuffer)` 메서드를 다음과 같이 구현하였다.
[source,java]
...
<3> `filter`(HexDecoder) 를 이용해 디코딩 한다.
<4> 버퍼에 읽어 담을 수 있는 개수보다 더 적게 읽히면 non-blocking channel 이라 가정하고 순환문을 빠져 나간다.
<5> 버버에 (디코딩 할) 바이트 개수가 홀수이면 하나를 더 읽어서 담는다.
...
주어진 값이 '0'(`48`)부터 '9'(`57`)까지의 문자이면 그 값에서 48을 뺀 값(`0` 부터 `9`)을, 대문자 'A'(`65`, `0x41`)부터 'F'(`70`, `0x46`) 까지의 값이면 그 값에서 `55` 를 뺀 값을, 그렇지 않으면 소문자 'a'(`97`) 부터 'f'(`102`) 까지의 값이라고 가정하고 그 값에서 87를 뺀 값을 반환한다. (앞서 인코딩을 구현할 때는 무조건 대문자로 변환하였지만 디코딩을 할 때는 대문자 및 소문자를 모두 처리한다.)
[CAUTION]
====
역시나 소문자로 가정한 부분이 있다. 다음과 같이 모든 경우에 대해 확실한 코드를 사용하면 좋겠다.
[source,java]
----
            case 'a': //  97, 0x61
            case 'b': //  98, 0x62
...
----
. `HexEncoderImplTest` 클래스에 테스트 메서드를 하나 추가 하고 클래스 파일의 magic 바이트를 `base16` 으로 출력해 보자.
+
[source,java]
----
    @Test
...
----
`{file}` 부분과 `{descriptor}` 부분에 `a/b` 와 `c&d=e` 을 각각 대입하면 유효하지 않거나 의도하지 않은 값이 나온다.
----
...
----
https://tools.ietf.org/html/rfc3986[RFC 3986]의 https://tools.ietf.org/html/rfc3986#section-2.1https://tools.ietf.org/html/rfc3986#section-2.1[Percent-Encoding] 항목에 위 예시에서 사용된 방법이 정의되어 있다. 주어진 문자가 https://tools.ietf.org/html/rfc3986#section-2.3[Unreserved Characters] 항목에 정의된 범위(알파벳, 숫자, `'-'`, `'.'`, `'_'`, `'~'`) 밖이면 `'%'` 문자 하나와 두개의 hex 문자로 표현한다.
...
public interface PercentEncoder {
    ...
    // 바이트를 인코딩 한 후 결과(한 개 혹은 세 개의 문자)를 버퍼에 담는다.
    // 버버에 인코딩된 문자들이 일부만 기록된 채 BufferOverflowException 을 던질 수 있다.
...
public interface PercentDecoder {
    ...
    // 버퍼에서 필요한 만큼(1개 혹은 3개)의 문자를 디코딩하고 그 결과를 반환한다.
    // 버퍼에 인코딩된 문자들이 일부만 사용된 채 BufferUnderflowException 을 던질 수 있다.
