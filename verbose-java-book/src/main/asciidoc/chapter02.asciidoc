...
final int len = available == 0 ? LEN : Math.min(LEN, available);
final byte[] buf = new buf[len];
----
<1> 이미 스트림의 끝(`end-of-stream`)에 도달한 상태라면 `0` 을 반환한다.
====
...
`HexEncoderImpl` 클래스의 기능을 확인하기 위한 시험클래스이다. 우선 아래와 같이 `Rfc4648TestVectors` 클래스를 `HexEncoderImpl` 클래스의 기능을 시험한다.
[source,java]
.HexEncoderImplTest.java#encodeVerboseCompareRfc4648TestVector()
----
    // 테스트 벡터의 입력값을 인코딩한 후 출력값을 비교한다.
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=37..42]
----

아래는 `Hex`(Commons-Codec) 클래스를 이용하여 `HexEncoderImpl` 클래스의 기능을 확인하는 시험항목이다.
[source,java]
.HexEncoderImplTest.java#encodeVerboseDecodeCommons()
----
    // HexEncoderImpl 클래스로 인코딩한 값을 Commons-Codec/Hex 클래스로 디코딩한 후 비교한다.
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=49..57]
----
<1> 인코딩에 사용할 배열을 생성한다.
<2> 두배의 크기를 가지는 배열을 생성한 후 `encode(ByteBuffer, ByteBuffer)` 메서드를 이용하여 인코딩 한다.
<3> 인코딩된 결과를 `Hex` 클래스를 이용하여 디코딩 한다.
<4> 인코딩에 사용한 배열과 디코딩된 결과를 비교한다.

아래는 `BaseEncoding`(Guava) 클래스를 이용하여 `HexEncoderImpl` 클래스의 기능을 확인하는 시험항목이다.
[source,java]
.HexEncoderImplTest.java#encodeVerboseDecodeGuava()
----
    // HexEncoderImpl 클래스로 인코딩한 값을 Guava/BaseEncoding 클래스로 디코딩한 후 비교한다.
...
----
<1> 인코딩에 사용할 입력 배열을 생성한다.
<2> `Hex` 클래스를 이용해 인코딩한다.
<3> 절반 크기의 배열을 생성한 후 `decode(ByteBuffer, ByteBuffer)` 메서드를 이용해 디코딩한다.
<4> 디코딩된 배열을 입력 배열과 비교한다.
