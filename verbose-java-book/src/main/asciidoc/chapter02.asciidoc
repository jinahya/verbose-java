...
아직 저 '`바보`' 보다 더 똑똑한 사람을 만난적은 없지만 프로그래밍을 할 때면 '`사람이 읽고 쓸 수 있는 문자`'와 '`기계가 읽고 쓸 수 있는 숫자`' 사이에 발생하는 문제를 해결해야 하는 경우가 종종 있다. 본 장에서는 이 '`문자`'와 '`숫자`' 사이의 관계에 대해 살펴본다.
...
`US-ASCII` 문자셋에는 줄바뀜(CR, LF), 탭(TAB) 등의 제어 문자들과 보통의 자판에서 찾아볼 수 있는 문자들을 포함하여 128(2^7^)개의 코드가 정의되어 있다. 때문에 여덟개의 비트로 표현할 수 있는 256(2^8^)개의 수에 각각의 문자를 대응하여 표현할 수 없다. 이를 해결하는 방법 중에 하나가 위에 있는 표의 가장 왼쪽 열에 있는 `base16` 로 표현하는 것이다. `base16` 표기는 여덟 개의 비트를 네 개씩 둘로 나눈 후에 네 개의 비트로 표현할 수 있는 열여섯개의 숫자에 각각의 문자를 대응시킨다.
...
`PercentEncoderImpl` 과 같이 `HexDecoder` 개체를 적재하는 코드를 작성하자.
[source,java]
