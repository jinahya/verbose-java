...
보통의 (거의 모든) 컴퓨터들은 자료를 저장할 때 여덟개의 비트로 구성된 최소 단위로 나누어 저장한다. 우리가 일반적으로 사용하는 모든 파일들(텍스트, 이미지, 오디오, 동영상 등)이 모두 이 `octet` 이라고 불리는 숫자들로 나뉘어 저장된다. 'a' 라는 문자는 (거의 모든 경우에) `01100001` 로, '홍' 이라는 문자는 (`UTF-8` 로 인코딩 되었을 때) `111011011 00110011 0001101` 로 저장된다. 아래 표에 여덟개의 비트로 표현할 수 있는 몇몇 값들을 10진법, 8진법, 2진법, 그리고 자바의 `byte` 값으로 표현하였다.
...
그리고 (네 개의 비트로 구성된) 각 그룹의 값을 아래 표에 정의된 열 여섯개(`2^4^`)의 문자 중 하나로 변경한다.
...
public interface HexEncoder {
    ...
    // 버퍼의 remaining 값이 2 보다 작으면 BufferOverflowException 을 던진다.
...
----
<1> `encoded` 버퍼에서 읽을 수 있는 바이트 개수의 절반 만큼의 버퍼를 생성하고 `encode(ByteBuffer, ByteBuffer)` 메서드에 전달한다.
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexOutputStream.java[lines=50..60]
----
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=117..125]
----
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=139..142]
----
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=155..165]
----
<1> `n` 을 기준으로 짝수만큼 시도한다.
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=38..50]
...
----
<1> 인코딩 버퍼의 크기는 최소한 2보다 커야 한다.

`write(ByteBuffer)` 메서드를 구현하기 전에 `java.nio.channels.Channel` 인터페이스에 정의된 두개의 메서드를 구현하였다.

[source,java]
.WritableHexChannel.java#isOpen()
----
    // channel.isOpen() 값을 반환한다.
    // channel 이 null 이면 IllegalStateException 을 던진다.
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=60..66]
----
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=75..80]
----
...
.WritableHexChannel.java#close()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=92..115]
----
...
`HexDecoder` 인터페이스를 이용하여 `read(ByteBuffer)` 메서드에 전달된 버퍼에 디코딩된 바이트들을 저장하는 채널이다. 생성자와 `isOpen()`, `close()` 등의 메서드는 `WritableHexChannel` 과 동일한 방식으로 구현되었다. 아래는 `read(ByteBufffer)` 메서드가 구현된 모습니다.
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=91..124]
}
----
