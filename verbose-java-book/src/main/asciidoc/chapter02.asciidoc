...
거의 모든 컴퓨터들은 자료를 저장할 때, `octet` 으로 불리는, 여덟개의 비트로 구성된 최소 단위의 숫자들로 나누어 저장한다. 'a' 라는 문자는 (거의 모든 경우에) `01100001` 로, '홍' 이라는 문자는 (`UTF-8` 로 인코딩 되었을 때) `111011011 00110011 0001101` 로 저장된다. 아래 표에 여덟개의 비트로 표현할 수 있는 몇몇 값들을 10진법, 8진법, 2진법, 그리고 자바의 `byte` 값으로 표현하였다.
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=40..53]
----
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=62..71]
----
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=81..92]
----
...
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=40..47]
----
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=56..62]
----
...
public class HexOutputStream extends FilterOutputStream {
    ...
    private ByteBuffer buf; // 인코딩에 사용할 버퍼
}
----
...
<2> `HexEncoder` 인터페이스의 `encodeOctet(int, ByteBuffer)` 메서드들 이용해 주어진 바이트를 변환한다.
<3> 변환된 두개의 hex 문자들을 `out` 에 기록한다.
...
final OutputStream hos = new HexOutputStream(null, null) {
    ...
    public void write(final byte b) {
        if (out == null) out = openFile();
        if (enc == null) enc = loadEncoder();
        super.write(b);
    }
};
----
주의 할 점은 `flush()`, `close()` 등의 메서드들이 `out` 필드가 `null` 인지 확인하지 않도록 구현되어 있다는 점이다.
[source,java]
...
다음과 같이 `read()` 메서드를 두 개의 바이트들을 읽고 하나로 디코딩한 후 반환하도록 재정의하였다.

[source,java]
.HexInputStream.java#read()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=51..70]
----
...
- http://docs.oracle.com/javase/8/docs/api/java/io/FilterInputStream.html#read-byte:A-int-int-[`FilterInputStream#read(byte[], int, int)`] 메서드는 `in` 필드에 직접 `read(byte[], int, int)` 메서드를 호출한다. 다음과 같이 재정의된 `read()` 메서드를 호출하도록 재정의되었다.
+
[source,java]
.HexInputStream.java#read(byte[], int, int)
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=85..106]
----
<1> `len` 만큼 `read()` 메서드가 `-1` 을 반환하기 전까지 호출한다. 첫 바이트가 `-1` 이면 `-1` 을 반환한다.

- 재정의된 `mark(int)` 메서드는 주어진 값의 두 배를 실제로 사용한다. (두개의 문자를 읽어서 하나로 반환하므로 두 배의 값을 사용한다.)
+
[source,java]
.HexInputStream.java#mark(int)
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=118..121]
----
CAUTION: `readlimit` 의 값이 `0` 보다 작거나 `Integer.MAX_VALUE / 2` 보다 클 경우의 결과는 예상할 수 없다.
...
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=135..138]
----
+
[CAUTION]
====
`InputStream#available()` 메서드가 반환하는 값을 최적의(optimal) 버퍼 크기로 오해하면 안된다. 몇몇 `InputStream` 들은 읽을 수 있는 모든 바이트들의 개수를 반환하다.
...
----
<1> `available()` 메서드가 파일(`src`)의 전체 크기를 반환할 수도 있다.
====
+
[TIP]
====
굳이 `InputStream#available()` 메서드가 반환하는 값을 사용하고자 한다면 다음과 같이 버퍼 크기의 최대치를 미리 정하고 비교한 후에 사용하자.
[source,java]
----
static final LEN = 4096; // maximum buffer length.
assert LEN > 0;

final int availalbe = in.available(); // <1>
final int len = available == 0 ? LEN : Math.min(LEN, available); // <2>
final byte[] buf = new buf[len];
----
<1> http://docs.oracle.com/javase/8/docs/api/java/io/InputStream.html#available--[`InputStream#available()`] 메서드는 이미 스트림의 끝(`end-of-stream`)에 도달한 상태라면 `0` 을 반환한다.
<2> http://docs.oracle.com/javase/8/docs/api/java/io/InputStream.html#read-byte:A-[`InputStream#read(byte[\])`] 메서드의 내용을 잘 읽어 보면, 크기가 `0` 인 버퍼를 전달했을 때 `end-of-stream` 상태인 것과는 상관 없이 `0` 을 반환한다고 한다. 때문에 크기가 `0` 인 버퍼를 사용하는 경우가 발생하지 않도록 주의해야 한다.
====

- `skip(long)` 메서드는 주어진 값보다 최대 1이 작은 짝수만큼 시도하고 홀수만큼 건너뛰었으면 무조건 하나를 더 소비한다.
+
[source,java]
.HexInputStream.java#skip(long)
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexInputStream.java[lines=152..162]
----
...
==== HexChannel.java
이제 `HexOutputStream` 클래스와 `HexInputStream` 클래스가 수행하는 기능을 `java.nio.channels` 패키지에 정의된 클래스들을 사용하여 구현해 보자. `FilterOutputStream` 과 `FilterInputStream` 에 해당하는 클래스들이 존재하지 않으므로 `WritableByteChannel` 과 `ReadableByteChanel` 을 직접 구현해야 한다. 다만 두 인터페이스가 공통으로 확장하는 `java.nio.channels.Channel` 인터페이스를 기반으로 추상클래스를 하나 정의하고 공통의 기능을 미리 구현하도록 하겠다.
...
public abstract class HexChannel<T extends Channel, U> implements Channel {
    ...
    // channel : 인코딩된 데이터가 기록되거나 읽혀질 채널
    // filter  : 인코딩/디코딩에 사용할 필터
    // capacity: 인코딩/디코딩에 사용할 버퍼의 용량
    // direct  : true 이면 버퍼를 생성할 때 다이렉트 버퍼를 생성한다
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=40..47]
...
http://docs.oracle.com/javase/8/docs/api/java/nio/channels/Channel.html[`Channel`] 인터페이스에 정의된 http://docs.oracle.com/javase/8/docs/api/java/nio/channels/Channel.html#isOpen--[`isOpen()`] 메서드는 `channel` 의 `isOpen()` 을 호출하도록 구현되었다.
[source,java]
.HexChannel.java#isOpen()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=58..61]
----
<1> `channel` 이 `null` 이면 `NullPointerException` 이 발생한다.

http://docs.oracle.com/javase/8/docs/api/java/nio/channels/Channel.html[`Channel`] 인터페이스에 정의된 http://docs.oracle.com/javase/8/docs/api/java/nio/channels/Channel.html#close--[`close()`] 메서드는 `channel` 이 `null` 이 아닐 경우에 한해 `channel` 에 직접 호출하도록 구현되었다.
[source,java]
.HexChannel.java#close()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=71..76]
----

`capacity` 와 `direct` 값을 사용하여 생성된 버퍼를 반환하는 메서드이다.
[source,java]
.HexChannel.java#buffer()
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexChannel.java[lines=84..91]
----

==== WritableHexChannel.java

`HexEncoder` 인터페이스를 이용해 `write(ByteBuffer)` 메서드에 전달된 버퍼의 값을 자동으로 인코딩하여 기록하는 체널이다.

[source,java]
.WritableHexChannel.java
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=27..29]

include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=39..47]
}
----
<1> 인코딩에 사용할 버퍼의 크기는 최소한 `2` 보다 커야 한다.

http://docs.oracle.com/javase/8/docs/api/java/nio/channels/WritableByteChannel.html[`WritableByteChannel`] 인터페이스에 정의된 http://docs.oracle.com/javase/8/docs/api/java/nio/channels/WritableByteChannel.html#write-java.nio.ByteBuffer-[`write(ByteBuffer)`] 메서드를 다음과 같이 구현하였다.
[source,java]
.WritableHexChannel.java#write(ByteBuffer)
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=59..77]
----
<1> `filter` 를 이용해 `src` 의 내용을 `buffer()` 에 인코딩한다.
<2> 인코딩된 값을 `channel` 에 기록한다.
<3> `buffer` 에 남은 데이터가 한번에 모두 기록되지 않았다면 `channel` 이 non-blocking mode 의 채널이라 가정하고 반복 구문을 빠져 나간다.
<4> `buffer` 에 (이미 인코딩된) 남아 있는 모든 데이터를 `channel` 에 기록한다.
...
.ReadableHexChannel.java
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=28..30]

include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=40..48]
----
<1> 디코딩 버버의 크기는 최소한 `2` 보다 커야 한다.

http://docs.oracle.com/javase/8/docs/api/java/nio/channels/ReadableByteChannel.html[`ReadableByteChannel`] 인터페이스에 정의된 http://docs.oracle.com/javase/8/docs/api/java/nio/channels/ReadableByteChannel.html#read-java.nio.ByteBuffer-[`read(ByteBuffer)`] 메서드를 다음과 같이 구현하였다.
[source,java]
.ReadableHexChannel.java#read(ByteBuffer)
----
include::../../../../hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=58..90]
----
<1> `src` 에 담을 수 있는 개수 보다 더 많이 읽지 안도록 설정한다.
<2> 처음에 `-1`(end-of-stream) 이 읽히면 `-1` 을 반환한다.
<3> `filter` 를 이용해 디코딩 한다.
<4> 버퍼에 담을 수 있는 개수보다 더 적게 읽혔으면 non-blocking channel 이라 가정하고 순환문을 빠져 나간다.
<5> 버버에 (디코딩 할) 바이트 개수가 홀수이면 하나를 더 읽어 담는다.
<6> 버퍼이 남아 있는 모든 바이트들을 디코딩한다.
...
----
<1> octet을 오른쪽으로 `4`(`0b100`) 만큼 쉬프트한 후 `15`(`017`) 와 AND(`&`) 연산하여 상위 네 개의 비트가 표현하는 첫번째 nibble을 구한다.
<2> octet과 `15`(`0xF`) 를 AND(`&`) 연산하여 하위 네 개의 비트가 표현하는 두번째 nibble을 구한다.
...
주어진 값이 '0'(`48`)부터 '9'(`57`)까지의 문자이면 그 값에서 `48` 을 뺀 값(`0` 부터 `9`)을, 대문자 'A'(`65`, `0x41`)부터 'F'(`70`, `0x46`) 까지의 값이면 그 값에서 `55` 를 뺀 값을, 그렇지 않으면 소문자 'a'(`97`) 부터 'f'(`102`) 까지의 값이라고 가정하고 그 값에서 `87` 를 뺀 값을 반환한다. (앞서 인코딩을 구현할 때는 무조건 대문자로 변환하였지만 디코딩을 할 때는 대문자 및 소문자를 모두 처리한다.)
[CAUTION]
====
역시나 소문자로 가정한 부분이 있다. 다음과 같이 모든 경우에 대해 확실한 코드를 사용하는 편이 더 훌륭한 코드이다.
[source,java]
----
            case 'a': //  97, 0x61
            case 'b': //  98, 0x62
...
`HexEncoderImpl` 클래스와 `HexDecoderImpl` 클래스가 올바르게 구현되었고 정상적으로 동작한다는 것을 어떻게 시험해야 할까? 단순히 시험에 사용할 데이터를 생성하고 `HexEncoderImpl` 객체를 이용해 인코딩한 후 다시 `HexDecoderImpl` 객체로 디코딩한 값을 비교하면 되지 않을까? 이 경우에 결과적으로 처음 입력한 데이터와 같은 결과를 얻었다 하더라도 두 클래스가 올바르게 작성되었다고 볼 수 없다. 두 클래스 모두 숫자 `15` 에 `'F'` 가 아닌 `'G'` 를 사용했더라도 시험을 통과하게 된다. 때문에 `HexEncoderImpl` 클래스와 `HexDecoderImpl` 클래스의 기능을 독립적으로 분리하여 각각의 기능을 비교해서 시험해 볼 수 있는 별도의 자료 혹은 라이브러리를 사용할 필요가 있다.

===== RFC 4648 The Base16, Base32, and Base64 Data Encodings
{rfc-4648}[RFC 4648] 의 http://tools.ietf.org/html/rfc4648#section-10[10. Test Vectors] 항목에 우리가 사용할 수 있는 테스트 벡터가 정의되어 있다. 아래와 같이 클래스를 하나를 만들고 시험 항목에서 사용할 수 있도록 한다.
[source,java]
.Rfc4648TestVectors.java#BASE16
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/Rfc4648TestVectors.java[lines=29..37]
----
`BASE16` 필드를 직접 노출하지 않고 다음과 같이 메서드를 하나 추가하여 데이터를 사용할 수 있게 한다.
[source,java]
.Rfc4648TestVectors.java#base16(BiConsumer)
----
    // 주어진 BiConsumer 의 accpet(String, String) 메서드에 BASE16 리스트의 값들을 전달한다.
    // 첫 번째 인자에은 BASE16 리스트의 짝수 번째 문자열을,
    // 두 번째 인자에는 BASE16 리스트의 홀수 번째 문자열을 제공한다.
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/Rfc4648TestVectors.java[lines=45..49]
----

===== Apache Commons Codec
{project-commons-codec}[Apache Commons Codec] 라이브러리의 {javadoc-commons-codec}/org/apache/commons/codec/binary/Hex.html[`Hex`] 클래스를 이용하여 인코딩/디코딩 결과를 교차 확인한다.

===== Guava: Google Core Libraries for Java
{project-guava}[Guava] 라이브러리에 포함된 {javadoc-guava}/com/google/common/io/BaseEncoding.html[`BaseEncoding`] 클래스를 이용하여 인코딩/디코딩 결과를 교차 확인한다.
...
[source,java]
.HexEncoderImplTest.java#encodeVerboseCompareRfc4648TestVector()
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=37..42]
----
<1> 테스트 벡터에 정의된 값(`d`)을 인코딩한 후 제시된 값(`e`)과 비교한다.
...
<3> 인코드된 결과를 `Hex` 클래스를 이용하여 디코드한다.
...
----
<1> `BaseEncoding`  클래스의 `decode` 메서드는 `CharSequence` 형의 인자를 받고 `byte[]` 형의 결과를 반환한다.
...
[source,java]
.HexDecoderImplTest.java#decodeVerboseCompareRfc4648TestVector()
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexDecoderImplTest.java[lines=36..41]
----
<1> 테스트 벡터에 정의된 값(`e`)을 디코딩한 후 제시된 값(`d`)과 비교한다.

`Hex` 클래스를 이용하여 `HexDecoderImpl` 클래스의 기능을 확인하자.
[source,java]
...
----
<1> 인코드할 배열을 생성한다.
<2> `Hex` 클래스를 이용해 인코드한다.
<3> 절반 크기의 배열을 생성한 후 `decode(ByteBuffer, ByteBuffer)` 메서드를 이용해 디코드한다.
<4> 처음 생성한 배열과 디코드된 배열을 비교한다.
...
----
<1> `BaseEncoding` 클래스의 `encode` 메서드는 `byte[]` 형의 입력값을 받고 `String` 형의 출력값을 반환한다.
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=53..61]
----
<1> 버퍼를 하나 생성하고
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=70..80]
----
<1> `flush()` 메서드를 명시적으로 호출하여 모든 데이터가 파일에 기록되는 것을 보장한다.

[NOTE]
====
`OutputStream` 객체 혹은 `Writer` 객체에 값을 기록한 후 `close()` 메스드를 호출하기 전에 `flush()` 메서드를 꼭 호출해야 한다. `write(XXX)` 메서드를 호출하는 것이 데이터가 디스크에 기록되거나 원격의 서버에 전달되는 것을 보장하지는 않는다.
====
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=82..96]
----
<1> `ReadableByteChannel#read(ByteBuffer)` 메서드가 `-1`(`end-of-stream`) 을 반환하지 않는 동안
<2> 퍼프를 뒤집어 (버퍼에 읽혀진 데이터를) 기록할 수 있는 상태로 만든 후
<3> `WritableByteChannle#write(ByteBuffer)` 메서드에 버퍼를 전달하여 기록한다.
<4> `writable` 객체가 `non-blocking` 모드일 경우 버퍼의 내용이 모두 기록되지 않았을 수도 있으므로 버퍼에 남아 있는 내용을 앞쪽으로 모은다.
<5> 버퍼에 남아 있는 내용을 모두 전달한다.

****
`copy1(ReadableByteChannel, WritableByteChannel)` 메서드는 `write(ByteBuffe)` 메서드를 한 번만 호출하는 대신 `compact()` 메서드를 사용해서 데이터를 계속 버퍼의 앞쪽으로 모으는 작업을 한다. [참조: http://shop.oreilly.com/product/9780596002886.do[Java NIO, O'Reilly Media]]
****

[source,java]
.IoUtils.java#copy2(ReadableByteBuffer, WritableByteBuffer)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=98..111]
----
<1> `ReadableByteChannel#read(ByteBuffer)` 메서드가 `-1`(`end-of-stream`) 을 반환하지 않는 동안
<2> 버퍼를 뒤집어 (버퍼에 읽혀진 데이터를) 기록할 수 있는 상태로 만든 후
<3> 버퍼에 데이터가 남아 있는 동안 `WritableByteChannle#write(ByteBuffer)` 메서드에 버퍼를 전달하여 기록한다.
<4> 버퍼를 읽어 넣을 수 있는 상태로 만든다.
...
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=132..141]
----
...
[NOTE]
====
`FileChannel#force(boolean)` 메서드를 사용하는 대신 `FileChannel#open` 메서드에 {apidocs-javase}/java/nio/file/StandardOpenOption.html#DSYNC[`StandardOpenOption#DSYNC`], {apidocs-javase}/java/nio/file/StandardOpenOption.html#DSYNC[`StandardOpenOption#SYNC`] 등을 전달하여 자동으로 처리되게끔 할 수도 있다.
====
...
[source,java]
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=143..157]
----

다음은 `trasferFrom` 메서드를 사용해서 파일을 복사하는 메서드이다.
[source,java]
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/IoUtils.java[lines=159..173]
----

==== Using MessageDigest

주어진 두 파일의 내용이 같은지 어떻게 확인할 수 있을까? 두 파일을 처음부터 끝까지 바이트별로 비교하는 것도 가능한 방법중에 하나일 수 있다. 또 다른 방법은 {apidocs-javase}/java/security/MessageDigest.html[`java.security.MessageDigest`] 클래스를 이용하여 각 파일의 해쉬 값을 구한 후 두 값을 비교하는 것이다.

다음은 `InputStream` 객체와 알코리즘 이름을 받아서 해쉬값을 구하는 메서드이다.
[source,java]
.MdUtils.java#digest1(InputStream, String)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=39..49]
----
<1> `algorithm` 값을 이용하여 `MessageDigest` 객제를 생성한다.
<2> `input` 에서 데이터를 읽을 후 `digest` 에 갱신한다.
<3> `digest` 에서 최종의 해쉬값을 계산하여 반환한다.

다음은 `DigestInputStream` 클래스를 사용하는 방법이다.
[source,java]
.MdUtils.java#digest2(InputStream, String)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=51..58]
----
<1> 주어진 `input` 객제를 `DigestInputStream` 으로 감싼다.
<2> `input` 의 모든 데이터를 읽는다. (내부적으로 읽힌 데이터가 `digest` 에 갱신된다.)
<3> `digest` 에서 최종의 해쉬값을 계산하여 반환한다.

이제 주어진 파일의 해쉬값을 구하는 메서드를 의외로 간단하게 구현될 수 있다.
[source,java]
.MdUtils.java#digest(File, String)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=91..96]
----

다음은 `ReadableByteChannel` 객체에서 데이터를 읽어서 해쉬값을 계산하는 메서드이다.

[source,java]
.MdUtils.java#digest(ReadableByteChannel, String)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=108..119]
----
<1> `channel` 이 끝에 도달하기 전까지, 반복해서,
<2> `buffer` 에 읽혀 담겨진 데이터를 `digest` 에 갱신한다.

역시나 `Path` 객체의 해쉬값을 구하는 메서드를 간단하게 구현할 수 있다.

[source,java]
.MdUtils.java#digest(Path, String)
----
include::../../../../hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/MdUtils.java[lines=130..136]
----


==== HexCodecImplTest.java

이제, 앞서 언급했듯이, Commons Codec 라이브러리와 Guava 라이브러리를 이용하여 교차 확인하는 시험 항목들을 작성해 보자.
...
----
. 시험 클래스를 하나 추가하고 클래스 파일의 magic 바이트를 `base16` 으로 출력해 보자.
+
[source,java]
.ClassFileTest.java
----
public class ClassFileTest {
    ...
    public void cafebabe() throws IOException {
        ...
        try (DataInputStream input
            ...
            new HexEncoderImpl().encode(ByteBuffer.wrap(decoded),
