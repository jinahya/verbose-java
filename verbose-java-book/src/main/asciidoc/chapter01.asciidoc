...
----
가능한 한 Hello World로 첫 장을 시작하고 싶지는 않았지만 프로젝트를 설계하고 구현하는 방법을 알아보기에 그 간단하고 명료한 요구사항이 적절하다고 판단되어 이 주제를 선택하였다. 이 장에서는 인터페이스를 정의하는 방법을 알아보고 그 인터페이스를 구현하고 사용하는 방법에 대해 알아본다.
...
<3> `buffer` 가 내부적으로 배열을 사용한다면 `set(byte[], int)` 메서드를 사용하여 그 배열에 값을 기록한 후 버퍼의 위치(position)를 임의로 `BYTES` 만큼 증가시킨 후 반환하다.
<4> 배열을 생성하고 `set(byte[], int)` 메서드를 이용해 값을 채운 다음 버퍼에 담은 후 반환한다.

[source,java]
.HelloWorld#write(InputStream)
----
...
<2> 배열을 생성하고 `set(byte[], int)` 메서드를 이용해 값을 채운 다음 스트림에 기록한 후 그 스트림을 반환한다.

[source,java]
.HelloWorld#write(WritableByteChannel)
----
...
----
시험을 수행하기 위해서는 (`default` 메서드들을 호출하고 결과를 확인해 볼 수 있는) `HelloWorld` 객체들이 필요하다. 다만 여기서 시험해 볼 범위는 `default` 메서드들 자체에 한정한다. `set(byte[], int)` 메서드가 어떻게 구현되었는지 확인할 필요는 없다.
...
[NOTE]
====
`HelloworldTest#impl()` 메서드는 `HelloWorld` 객체를 반환하기 위해 lambda 표현식을 사용하였다. 여기이서 사용된 lambda 표현식을 이해하기 위해 동일한 기능을 수행하는 방식들을 따라가 보자.
...
@Override
public void set(final byte[] array, final int offset) {
    if (offset < 0) // throw IndexOutOfBoundsException
    if (array == null) // throw NullPointerException
    if (offset + HelloWorld.BYTES > array.length) // throw IndexOutOfBoundsException
}
----
...
`HelloWorld` 객체들은 주입받는 클래스를 정의하자.
...
[source,java]
.HelloWorldServiceProviderTest.java#implementation()
----
@Override
HelloWorld implementation() {
