...
----
가능한 한 Hello World로 첫 장을 시작하고 싶지는 않았지만 프로젝트를 설계하고 구현하는 방법을 알아보기에 그 간단하고 명료한 요구사항이 적절하다고 판단되어 이 주제를 선택하였다. 이 장에서는 인터페이스를 정의하는 방법을 알아보고 그 인터페이스를 구현하고 사용하는 방법에 대하 알아본다.

=== hello-world-api
우리가 앞으로 구현해 볼 인터페이스가 포함되어 있는 모듈이다.
...
public interface HelloWorld {
    ...
    // (offset + BYTES)가 array.length보다 크면 IndexOutOfBoundsException을 던진다.
...
----
    ...
    // 버퍼의 위치(position)가 BYTES 만큼 증가된다.
include::../../../../hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=66..82]
----
<1> `buffer` 인자가 `null` 이면 `NullPointerException` 을 던진다.
<2> `buffer` 에 충분한 공간이 없으면 `IllegalArgumentException` 을 던진다.
<3> `buffer` 가 내부적으로 배열을 사용한다면 `set(byte[], int)` 메서드에 그 배열을 사용한다.
<4> 배열을 생성하고 `set(byte[], int)` 메서드를 이용해 값을 채운 다음 버퍼에 담는다.

[source,java]
.HelloWorld.java#write(InputStream)
----
...
<2> 배열을 생성하고 `set(byte[], int)` 메서드를 이용해 값을 채운 다음 스트림에 기록한다.
...
<3> 버퍼에 기록할 값이 남아있는 동한 `channel` 에 기록한다.

NOTE: `WritableByteChannel` 을 상속한 클래스들 중에는 내부적인 상태 혹은 설정에 따라 http://docs.oracle.com/javase/8/docs/api/java/nio/channels/WritableByteChannel.html#write-java.nio.ByteBuffer-[`write(ByteBuffer)`] 메서드에서 퍼버의 사용 가능한 모든 바이트들을 한번에 기록하지 않을 수도 있다.

==== HelloWorldTest.java
인터페이스를 위해 단위시험을 수행한다는 것이 의외일 수도 있지만 `HelloWorld` 인터페이스에 구현된 `default` 메서드들이 정상적으로 동작하는지 미리 확인해 둘 필요가 있다.
...
----
시험을 수행하기 위해서는 (`default` 들을 호출하고 결과를 확인해 볼 수 있는) `HelloWorld` 형의 객체들이 필요하다. 앞서 보았듯이 `default` 메서드들 결과적으로 `set(byte[], int)` 메서드를 이용한다. 하지만 여기서 시험하고자 하늘 범위는 `default` 메서드들 자체에 한정한다. `set(byte[], int)` 메서드가 어떻게 구현되었는지 확인할 필요는 없다.

`defualt` 메서드들을 시험하기 위해 다음과 같이 `HelloWorld` 객체를 반환하는 메서드를 정의하자.
[source,java]
.HelloWorldTest#impl()
----
include::../../../../hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=53..56]
----
<1> 아무런 동작도 하지 않는다.

[NOTE]
====
`HelloworldTest#impl()` 메서드는 `HelloWorld` 객체를 반환하기 위해 lambda 표현식을 사용하였다. 여기이서 사용된 lambda 표현식을 이해하기 위해 아래 동일한 기능을 수행하는 방식들을 따라가 보자.

우선 다음과 같이 `HelloWorld` 인터페이스를 구현하는 클래스를 정의하고 그 객체를 반환할 수 있다.
[source,java]
----
    private static class Impl implements HelloWorld {
        @Override public void set(final byte[] array, final int offset) {
            // empty
        }
    }
...
----
lambda 표현식에서 `set(byte[], int)` 메서드에서 사용되는 인자들(`byte[]`, `int`)의 형(type)이 추론(infer)될 수 있으므로 다음과 같이 형이 생략될 수 있다.
[source,java]
----
    private HelloWorld impl() {
        return (a, o) -> {
...
====
`set(byte[], int)` 메서드를 다음과 같이 구현했다고 가정하자.
[source,java]
----
    private HelloWorld impl() {
        final byte[] bytes = "hello, world".getBytes(US_ASCII);
        return (a, o) -> {
            System.arraycopy(bytes, 0, a, 0, bytes.length);
        };
    }
----
단 하나의 서술문(statement)만 사용되었으므로 다음과 같은 표현이 가능하다.
[source,java]
----
    private HelloWorld impl() {
        final byte[] bytes = "hello, world".getBytes(US_ASCII);
        return (a, o) -> System.arraycopy(bytes, 0, a, 0, bytes.length);
    }
----
TIP: 맨 마지막에 사용된 세미콜론(`;`)은 `return` 구문의 끝맺음을 나타내는 문자이다. (`System.arraycopy(...)` 서술문의 끝맺음을 나타내는 것이 아니다.)
====
...
.HelloWorldTest#put()
----
include::../../../../hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=61..79]
----
<1> `buffer` 값이 `null` 이면 `NullPointerException` 을 던지는지 확인한다.
<2> `buffer` 에 충분한 공간이 없으면 `IllegalArgumentException` 을 던지는지 확인한다.
<3> `byte[]` 를 사용하는 `buffer` 를 가지고 확인한다.
<4> `byte[]` 를 사용하지 않는 `buffer` 를 가지고 확인한다.

보는 바와 같이 `put(ByteBufer)` 메서드가 에러 없이 동작하는 지를 시험했을 뿐 전달한 `buffer` 에 어떤 값이 들어갔는지 확인하지 않는다.

[NOTE]
====
`org.testng.Assert.assertThrows` 메서드의 두번째 인자의 형인 `ThrowingRunnable` 인터페이스는 단 하나의 메서드만이 정의되어 있다. 아래와 같은 코드들이 아닌 최소한의 lambda 표현식이 사용되었다.
[source,java]
----
...
[source,java]
.HelloWorldTest#writeWithStream()
----
include::../../../../hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=86..93]
----
...
[source,java]
.HelloWorldTest#writeWithChannel()
----
include::../../../../hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=100..108]
----
<1> `channel` 값이 `null` 이면 `NullPointerException` 을 던지는지 확인한다.
<2> 유효한 `WritableByteChannel` 값으로 `write(WritableByteChannel)` 을 호출한다.

=== hello-world-lib
위의 hello-world-api 모듈에 정의된 API를 구현할 모듈이다. 다음과 같이 의존성을 확인해 보자.
...
----
때문에 첫번째 항목(`array` 가 `null`)을 테스트할 경우에는 두번째 조건에 위배되지 않는 `offset` 값을 사용해야 하며 두번째 항목(`offset` 이 음수)을 검증할때는 `array` 의 값으로 `null` 을 사용하면 안된다. 다음 테스트는 `array` 가 `null` 일 경우와 `offset` 이 음수일 경우를 한꺼번에 시험한다.
...
테스트 주도 개발(http://en.wikipedia.org/wiki/Test-driven_development[Test-driven development])이라고 하는 방법론을 적용해 보려 한다. 코드가 어떻게 동작해야 하는지에 대한 정확한 요구사항을 바탕으로 최소한의 범위에 한정하는 (그리고 **실패하는**) **시험 항목을 먼저 작성**한 후 그 **시험 항목이 성공할 만큼만 구현/수정** 작업을 수행한다.
====
...
----
include::../../../../hello-world-lib/src/test/java/com/github/jinahya/verbose/hello/HelloWorldImplTest.java[lines=41..46]
----
...
.HelloWorldImplTest#expectIndexOutOfBoundsExceptionWhenOffsetIsNegative
[source,java,indent=0]
----
include::../../../../hello-world-lib/src/test/java/com/github/jinahya/verbose/hello/HelloWorldImplTest.java[lines=52..57]
----
...
$ mvn test
...
Method HelloWorldImplTest.expectIndexOutOfBoundsExceptionWhenOffsetIsNegative()[
...] should have thrown an exception of class java.lang.IndexOutOfBoundsException
...
...
.HelloWorldImplTest#expectIndexOutOfBoundsExceptionWhenCapacityIsNotEnough
[source,java,indent=0]
----
include::../../../../hello-world-lib/src/test/java/com/github/jinahya/verbose/hello/HelloWorldImplTest.java[lines=64..69]
----
...
$ mvn test
...
Method HelloWorldImplTest.expectIndexOutOfBoundsExceptionWhenCapacityIsNotEnough()[
...] should have thrown an exception of class java.lang.IndexOutOfBoundsException
...
...
[source,java,indent=0]
----
include::../../../../hello-world-lib/src/test/java/com/github/jinahya/verbose/hello/HelloWorldImplTest.java[lines=75..92]
----
...
[source,java,indent=0]
----
include::../../../../hello-world-lib/src/main/java/com/github/jinahya/verbose/hello/HelloWorldImpl.java[lines=24..49]
----

[CAUTION]
================================================================================
`offset` 인자에 `final` 키워드를 생략한 다음 그 값을 직접 증가(`++`)시키면서 사용하였다. 좋은 방법이 아니다. 중간에 `offset` 값을 변경하는 코드가 삽입될 경우 정상적으로 동작하지 않는다. 가능한 한 메서드에 전달된 인자들에 `final` 키워드를 사용하여 에러가 발생할 확률을 줄인다. [http://checkstyle.sourceforge.net/config_misc.html#FinalParameters[Checkstyle#FinalParameters]]
================================================================================
...
다음은 `HelloWorldDataProvider#provideOffsetNegative` 메서드가 제공하는 데이터를 사용하는 시험 항목이다.
...
[NOTE]
====
hello-world-lib 모듈의 테스트 클래스들중에는 `HelloWorld` 인터페이스를 구현한 `HelloWorldDemo` 라는 클래스가 있다. 보다 많은 의존성 주입 사례를 위해 우리가 이미 구현한 `HelloWorldImpl` 클래스와 같이 사용한다.
[source,java]
.HelloWorldDemo.java
----
...
--------------------------------------------------------------------------------
<1> `@javax.inject.Inject` 어노테이션을 상용함으로써 외부로부터 주입된다는 것을 선언하였다. `any` 필드에는 `HelloWorldDemo` 와 `HelloWorldImpl` 중 어느 구현체가 주입될지 알 수 없다.
...
Guice를 사용하여 주입작업을 수행하기 위해서는 실제로 주입을 필요로 하는 요소들(필드, 메서드 등)에 따른 주입작업을 설정하는 모듈 클래스가 필요하다. `com.google.inject.AbstractModule` 클래스를 구현하는 클래스를 하나 정의하자. 이 클래스는 Guice 프레임워크에 종속적인 API 를 사용한다.
...
`@Named("demo")` 혹은 `@Named("impl")` 이 적용된 필드들에는 각각의 약속된 형태를 주입한다.
...
hello-world-lib 모듈에는 Guice 이외에 [https://jcp.org/en/jsr/detail?id=330[JSR 330: Dependency Injection for Java]]을 지원하는 몆몆 프레임워크를 사용하는 별도의 테스트 클래스들이 추가되어 있다.

[cols="1,1,2",options="header"]
.JSR 330 Implementations and Test classes
|===============================================================================
...
$ pwd
/.../path/to/.../verbose-java
$ mvn -pl hello-world-lib -P<profile> clean test
...
$
----
...
=== hello-world-app
지금까지 우리는 성공적으로 hello-world-api 모듈을 구현하였다. 상세한 시험 항목을 통해 우리가 구현한 코드가 의도한데로 그리고 정상적으로 동작하는 것을 확인하였다. 또한 의존성 주입 및 서비스 확장이라는 개념까지 살펴보았다. 이제 우리는 우리가 직접 구현한 코드를 가지고 본 장 첫 부분에서 보았던 `SuccinctHelloWorld` 프로그램과 동일한 기능을 수행하는 프로그램을 만들 수 있다. 다음과 같이 의존성을 확인해보자.

[source,text]
----
$ pwd
/.../path/to/.../verbose-java
$ mvn -pl hello-world-app dependency:tree -Dscope=runtime
...
...
$
----
hello-world-app 모듈은 hello-world-api 모듈에 정의된 클래스들만을 (컴파일 시에) 사용하고 hello-world-lib 모듈은 실제로 동작할 때 사용된다. 아래는 hello-world-app 모듈의 pom.xml 파일에 정의된 의존성들을 보여준다.
[source,xml]
----
...
==== HelloWorldMain.java
우리가 작성한 `HelloWorldImpl` 을 이용하여 `hello, world` 를 인쇄하는 프로그램이다. 앞서 구현했던 서비스 확장 기능을 사용한다.
...
==== Executable JAR
`HelloWorldMain` 클래스를 실행하기 위해서는 hello-world-api 모듈과 hello-world-lib 모듈이 필요하다. hello-world-app 모듈은 `package` 단계(phase)에서 여러가지 방업을 통해 실행 가능한 jar 파일들을 생성한다.
...
===== class-path
실행에 필요한 모듈들을 별도의 디렉토리에 복사한 후 `META-INF/MANIFEST.MF` 항목에 별도의 `Class-Path` 값을 추가하는 방식으로 생성되었다. 다음과 같이 실행해 볼 수 있다.
[source,text]
----
$ pwd
/../path/to/../verbose-java/hello-world-app
...
$
----
다음과 같이 `META-INF/MANIFEST.MF` 항목의 확인해 보자.
[source,text]
----
$ pwd
/../path/to/../verbose-java/hello-world-app
...
...
Class-Path: hello-world-app-<version>-classpath.lib/hello-world-api-<v
ersion>.jar hello-world-app-<version>-classpath.lib/hello-world-lib-<v
ersion>.jar
...
$
----
...
. `HelloWorldImpl#set(byte[], int)` 메서드를 위한 유효한 데이터를 제공하는 데이터 제공자와 그로부터 데이터를 제공받아 정상적인 동작을 확인하는 시험 항목을 작성하라.
+
[source,java]
