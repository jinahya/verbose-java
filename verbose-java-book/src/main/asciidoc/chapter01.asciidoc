...
|capacityNotEnough|new byte[HelloWorld.BYTES]    | 1
|                 |new byte[HelloWorld.BYTES + 1]| 2
|                 |new byte[HelloWorld.BYTES + 2]| 3
|                 |new byte[HelloWorld.BYTES + 3]| 4
|valid            |new byte[HelloWorld.BYTES]    | 0
...
--------------------------------------------------------------------------------
`@javax.inject.Inject` 어노테이션을 상용함으로써 외부로부터 주입된다는 것을 선언하였다. `any` 필드에는 `HelloWorldDemo` 와 `HelloWorldImpl` 중 어느 구현체가 주입될지는 모른다. `namedDemo` 필드와 `namedImpl` 필드에는 `@javax.inject.Named` 어노테이션을 사용하여 미리 약속된 문자열(`"demo"`, `"impl"`)을 통해 원하는 형태의 구현체를 주입받는다. `qualifedDemo` 필드와 `qualifiedImpl` 필드는 `@javax.inject.Qualifier` 어노테이션이 적용된 별도의 어노테이션들을 사용하여 원하는 형태의 값을 주입받는다
