...
여기서 주의 할 점은 위에서 열거된 순서와 실제 코드에서 구현한 순서가 다를 수도 있다는 것이다. 세 번째 항목은 `array` 와 `offset` 이 모두 유효한 상황에서 검증해야 할 항목이지만 첫 번째 항목과 두번 째 항목은 서로 상관이 없다. 때문에 두 항목을 구현한 순서가 달라도 상관이 없다.
...
----
<1> http://docs.oracle.com/javase/8/docs/api/java/lang/System.html#arraycopy-java.lang.Object-int-java.lang.Object-int-int-[System#arraycopy(Object, int, Object, int, int)] 메서드가 `HelloWorld#set(byte[], int)` 메서드의 요구사항에 부합하는지 확인해보자.
====
