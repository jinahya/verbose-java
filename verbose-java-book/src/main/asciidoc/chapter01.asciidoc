...
----
가능한 한 Hello World로 첫 장을 시작하고 싶지는 않았지만 기본적인 프로젝트를 설계하고 구현하는 방법을 살펴보기 위해 이 주제를 선택하였다.
...
----
<1> 구현해야 할 메서드가 한 개밖에 없으므로 `@FunctionalInterface` 어노데이션을 달았다.
...
`HelloWorld` 인터페이스에는 위와 같이 정의된 한 개의 필드와 한 개의 메서드 외에 `default` 수정자가 적용된 메서드들이 정의되어 있다.

[source,java]
.HelloWorld.java#put(ByteBuffer)
----
    // 주어진 퍼버에 hello, world 바이트들을 담는다. 버퍼의 위치(position)가 BYTES 만큼 증가된다.
include::../../../../hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=65..82]
----
...
----
    // 주어진 스트림에 hello, world 바이트들을 기록한다.
include::../../../../hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=91..99]
----
...
----
    // 주어진 채널에 hello, world 바이트들을 기록한다.
include::../../../../hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=111..121]
----
<1> `channel` 인자가 `null` 이면 `NullPointerException` 을 던진다.
<2> 버퍼를 생성하고 `put(ByteBuffer)` 메서드를 이용해 값을 채운다.
<3> 버퍼에 기록할 값이 남아있을 동한 `channel` 에 기록한다.

NOTE: `WritableByteChannel` 을 상속한 클래스들 중에는 내부적인 상태 혹은 설정에 따라 `write(ByteBuffer)` 메서드에서 퍼버의 사용 가능한 모든 바이트들을 기록하지 않을 수도 있다.
...
public void do_not_ever_test_like_this() {
    ...
    try {
        ...
        // array 를 먼저 확인하는 군!
    } catch (final IndexOutOfBoundsException aioobe) {
        // offset 을 먼저 확인하는 듯?
    }
}
----
...
TIP: 테스트 주도 개발(http://en.wikipedia.org/wiki/Test-driven_development[Test-driven development])이라고 하는 방법론을 적용해 보려 한다. 코드가 어떻게 동작해야 하는지에 대한 정확한 요구사항을 바탕으로 최소한의 범위에 한정하는 (그리고 **실패하는**) **시험 항목을 먼저 작성**한 후 그 **시험 항목이 성공할 만큼만 구현/수정** 작업을 수행한다.
...
--------------------------------------------------------------------------------
<1> `@javax.inject.Inject` 어노테이션을 상용함으로써 외부로부터 주입된다는 것을 선언하였다. `any` 필드에는 `HelloWorldDemo` 와 `HelloWorldImpl` 중 어느 구현체가 주입될지는 모른다.
...
Guice를 사용하여 주입작업을 수행하기 위해서는 실제로 주입을 필요로 하는 요소들(필드, 메서드 등)에 따른 주입작업을 설정하는 모듈이 필요하다. `com.google.inject.AbstractModule` 클래스를 구현하는 모듈을 하나 정의하자.
...
=== hello-world-app
지금까지 우리는 성공적으로 hello-world-api 모듈을 구현하였다. 상세한 시험 항목을 통해 우리가 구현한 코드가 의도한데로 그리고 정상적으로 동작하는 것을 확인하였다. 또한 의존성 주입 및 서비스 확장이라는 개념까지 살펴보았다. 이제 우리는 우리가 직접 구현한 코드를 가지고 본 장 첫 부분에서 보았던 `SuccinctHelloWorld` 프로그램과 동일한 기능을 수행하는 프로그램을 만들 수 있다.
...
==== 실행 가능한 jar 파일 생성하기
`HelloWorldMain` 클래스를 실행하기 위해서는 hello-world-api 모듈과 hello-world-lib 모듈이 필요하다. 몇몇 플러그인을 이용하여 실행에 필요한 파일들을 모두 포함하는 실행 가능한 jar파일을 생성해 보자.

===== Apache Maven Assembly Plugin
다음은 http://maven.apache.org/plugins/maven-assembly-plugin/[Apache Maven Assembly Plugin]의 `jar-with-dependencies` descriptor를  이용하는 방법이다.

[source,xml,indent="0"]
.maven-assembly-plugin:single
----
include::../../../../hello-world-app/pom.xml[lines=49..71]
----
다음과 같이 실행해 볼 수 있다.
...
----
include::../../../../hello-world-app/pom.xml[lines=144..173]
----
...
[source,xml,indent="0"]
----
include::../../../../hello-world-app/pom.xml[lines=174..199]
----
...
****
위에서 생성된 jar 파일들의 `META-INF/MANIFEST.MF` 항목의 내용을 확인해보자.
[source,text]
----
$ pwd
/.../path/to/.../verbose-java
...
=== Exercises
****
`HelloWorldImpl#set(byte[], int)` 에서 `offset` 인자에 `final` 키워드를 추가한 다음 부가적으로 발생하는 문제점들을 해결하라.
[source,java]
----
public void set(final byte[] array, final int offset) {
}
